...
var btns_collection_tabs: Array[Button] = [] # set in _enter_tree()
# Options
var btn_use_surface_normal: Button
var btn_surface_normal_x: Button
var btn_surface_normal_y: Button
var btn_surface_normal_z: Button
...
func _enter_tree() -> void:
	...
	set_dock_tab_icon(scene_builder_dock,load("res://addons/scene_builder/3d-hammer.svg"))
	# Collection tabs
	tab_container = scene_builder_dock.get_node("%Items")
	for i: int in range(num_collections):
		var collection_tab = scene_builder_dock.get_node("%Collections")
		var tab_button: Button = collection_tab.get_node(str(i+1))
	...
	spinbox_translate_snap = scene_builder_dock.get_node("%TranslateSnap")
	spinbox_rotate_snap = scene_builder_dock.get_node("%RotateSnap")
	spinbox_scale_snap = scene_builder_dock.get_node("%ScaleSnap")
...
func reload_all_items() -> void:
	...
	for i in range(1, num_collections + 1):
		var grid_container: Container = tab_container.get_node("%s/Grid" % i)
	...
	for collection_name in collection_names:
		...
		if collection_name != "" and DirAccess.dir_exists_absolute(config.root_dir + "/%s" % collection_name):
			...
			for key: String in ordered_keys_by_collection[collection_name]:
				...
				texture_button.size_flags_horizontal = 3
...
func refresh_collection_names() -> void:
	...
	if _names != null:
			...
			for i in range(num_collections):
				...
				if collection_name == "":
					#collection_name = " "
					btns_collection_tabs[i].visible = false
				else:
					btns_collection_tabs[i].visible = true
